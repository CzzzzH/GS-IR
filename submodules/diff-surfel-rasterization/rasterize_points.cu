	...
	const bool debug)
{
  ...
  return std::make_tuple(rendered, out_color, out_others, out_albedo, out_roughness, out_metallic, radii, geomBuffer, binningBuffer, imgBuffer);
}
	...
	const bool debug) 
{
  ...
  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dopacity, dL_dalbedo, dL_droughness, dL_dmetallic, dL_dmeans3D, dL_dtransMat, dL_dsh, dL_dscales, dL_drotations);
}
